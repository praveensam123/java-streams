...
public class EmployeeServiceTest {
    ...
    public void find_employee() {
        Employee employee = service.findAnyEmployee();
        assertThat(employee.toString(), equalTo("Bob Martin R"));
    }
    ...
    @Test
    public void skip_first_employee_and_get_first_three_employee_names() {
        List<Name> employees = service.skipFirstEmployeeAndGetFirstThreeEmployeeNames();
        assertThat(employees.size(), equalTo(3));
        assertThat(employees.get(0).toString(), equalTo("James Matthew S"));
        assertThat(employees.get(1).toString(), equalTo("Sharmila  D"));
        assertThat(employees.get(2).toString(), equalTo("Christine Sophia G"));
    }
    ...
    @Test
    public void find_any_employee_matches_with_name() {
        String firstName = "Anitha";
        boolean matchFound = service.matchEmployeeFirstName(firstName);
        assertTrue(matchFound);
    }

    @Test
    public void ensure_employee_name_does_not_match_with_all_employees() {
        String firstName = "Rajesh";
        boolean noMatchFound = service.doesNotMatchEmployeeLastName(firstName);
        assertTrue(noMatchFound);
    }

    @Test
    public void employees_should_have_firstName() {
        boolean hasFirstNames = service.checkAllEmployeesHaveFirstName();
        assertTrue(hasFirstNames);
    }
